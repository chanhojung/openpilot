...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators, pcm_cancel_cmd, visual_alert,
    ...
    # 차간거리 속도 연동 변환하기
    if CS.acc_active and (CS.out.vEgo * CV.MS_TO_KPH)+5 < 30 : #GAP_DIST 1칸 만들기
      ...
      if CS.cruiseGapSet == 2.0:
        self.cruise_gap_auto_switch_timer = 0          
    elif CS.acc_active and (CS.out.vEgo * CV.MS_TO_KPH)+5 < 100 :#GAP_DIST 3칸 만들기
      self.cruise_gap_auto_switch_timer += 1
      ...
      if CS.cruiseGapSet == 3.0:
        self.cruise_gap_auto_switch_timer = 0          
    elif CS.acc_active and (CS.out.vEgo * CV.MS_TO_KPH)+5 >= 100: #GAP_DIST 4칸 유지들기
      self.cruise_gap_auto_switch_timer += 1
